...
export class ListComponent {
  ...
  getPokemonList(offset:number, limit:number) {
    this.pokemonService.getPokemonList(offset, limit).then(response => {
      this.pagination = this.paginationService.getPagination(response.count, limit);
  ...
  changePage(page: number) {
    this.searchText = '';
    this.pagination = this.paginationService.getPagination(this.pagination.pages, this.limit, page);
  ...
  searchOnList() {
    this.filterableList = this.list.filter((pokemon: any) => pokemon.name.includes(this.searchText))
  }
